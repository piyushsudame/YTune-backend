...
from app.routes.ytmusic.playlists import get_playlists, get_playlist_details
...
def playlists():
    try:
        ...
        if not playlists:
            return jsonify({'message': 'No playlists found', 'playlists': []})
...
def playlist_details(playlist_id):
    ...
    try:
        ...
        error_message = str(e)
        if "not found or is not accessible" in error_message:
            return jsonify({'error': error_message}), 404
        return jsonify({'error': f'Error fetching playlist details: {error_message}'}), 500
